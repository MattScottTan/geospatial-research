"""
build_fig08_case_studies.py

Generates the four case-study spotlight figures for Section 8:
  08_case_filipino.png  — Pacific-archipelagic bridge
  08_case_russian.png   — Long-distance Eurasian bridge
  08_case_thai.png      — East/SE Asian regional hub
  08_case_spanish.png   — Iberian/Atlantic-Pacific node

Each figure follows Cloudy with a Chance of Compute's case-study pattern:
left panel = regional spotlight map showing the cuisine anchor and its top
residual partners with great-circle connections; right panel = scorecard
with top residual partners as a bar chart and a stats summary box (mean
residual, bridge index, LISA classification, role in network).

USAGE
-----
This script depends on:
  - The cuisine-by-ingredient matrix at ./cuisine_ingredient_matrix.csv
  - Natural Earth shapefiles at /home/claude/ne_repo
  - pre-computed similarity_matrix.npy, distance_matrix.npy,
    residual_matrix.npy, mean_resid.npy, cuisines.txt, lisa_results.json
    (all produced by build_fig07_lisa_and_mantel.py)

Run build_fig07_lisa_and_mantel.py first to generate the LISA outputs,
then run this script. Outputs are written to the working directory.
"""
import json
import numpy as np
...
def build_case_figure(focal, cfg):
    ...
    land  = ShapelyFeature(shpreader.Reader(f'{NE}/110m_physical/ne_110m_land.shp').geometries(),
                           ccrs.PlateCarree(), facecolor=LAND_COLOR, edgecolor='none')
    ocean = ShapelyFeature(shpreader.Reader(f'{NE}/110m_physical/ne_110m_ocean.shp').geometries(),
                           ccrs.PlateCarree(), facecolor=OCEAN_COLOR, edgecolor='none')
    coast = ShapelyFeature(shpreader.Reader(f'{NE}/110m_physical/ne_110m_coastline.shp').geometries(),
                           ccrs.PlateCarree(), facecolor='none', edgecolor=COAST_COLOR, linewidth=0.4)
    borders = ShapelyFeature(shpreader.Reader(f'{NE}/110m_cultural/ne_110m_admin_0_countries.shp').geometries(),
