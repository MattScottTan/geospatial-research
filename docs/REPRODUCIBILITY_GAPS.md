...
`../eip/submission/award-materials/README.md`.

**compute-atlas, Findings 1–2 — code found.** `cloudy_compute_session_export.zip`
contained two scripts, now at `eip/analysis/`:
...
`code/pipeline.py` implements both statistics itself, in numpy/scipy:
...
shipped code, and `data/gis/cities_with_hotspots.geojson` is that code's output.

`analysis/spatial_diagnostics_esda.py` ports both to `esda`/`libpysal` and verifies
...
`eip/submission/award-materials/methods/SPATIAL_STATS_ARCGIS_INSTRUCTIONS.md` specifies "K Nearest Neighbors,
...
`bridges_final_package.zip` and now sits at `fisher/analysis/`:
...
see `../fisher/submission/award-materials/README.md` for the claim-by-claim table.
...
Still true of the file itself: `fisher/submission/versions/fisher-submission/code/figdata.py` hardcodes
...
(`analysis/step4d_final_figure.py`, `code/figure-builders/build_fig07_lisa_and_mantel.py`,
`code/figure-builders/build_fig08_case_studies.py`). Prefer those; treat `figdata.py` as a superseded
early build helper.
...
graph of `code/pipeline.py` and the culinary figure builders. It is therefore a
...
`code/pipeline.py` imports neither. It uses
...
ArcGIS Online / Pro. This is better documented than I first thought — `eip/submission/award-materials/methods/`
...
Both projects' spatial statistics reproduce. Items 1 and 2 of the original priority list
are done (§1). What remains:

1. **Reconcile the eip weighting discrepancy** — either amend the methods document to say
   symmetrized binary, or re-run at the documented row-standardised specification and
   update the hot/cold map. See §1.
2. **Correct the city-frame attribution** — SimpleMaps, not Natural Earth. See
   `DATA_PROVENANCE.md`.
3. **Re-fit the eip GP and CAR with full posteriors** so the factor-of-four coefficient
   gap can be diagnosed rather than just noted. See §4.
4. **Establish `cuisine_ingredient_matrix.csv` provenance** before the repo could ever go
   public. See `DATA_PROVENANCE.md`.
