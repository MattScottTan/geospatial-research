...
`scripts/bootstrap_repo.ps1` performs the original copies. Destination paths below are
given post-restructure; the script still writes the old `projects/` layout and would
need repointing before a re-run.

## eip/  (was projects/compute-atlas)
...
| Destination | Source |
|---|---|
| `code/` | `Downloads/AI_Compute_Accessibility_Atlas_EIP_Submission_Package Part 4/src/` |
...
| `docs/` | `…_Package/docs/` |
| `analysis/extensions/` | `…_Package/extensions/` (stage4, stage5, stage6) |
| `results/deliverables/` | `…_Package/deliverables/` |
| `results/report/` | `…_Package Part 2/report/` |
| `Makefile`, `Makefile.local` | `…_Package Part 2/` |
| `requirements-extended.txt` | `…_Package Part 2/` |
| `submission/original-package/` | `…_Package Part 3/final_submission/` |
| `submission/original-package/originality/` | `…_Package Part 6/final_submission/originality/` |
| `code/openalex/` | `Documents/Projects/openalex_overlay/` (excluding `.venv`) |
...
copy each, into `eip/submission/award-materials/` — see that directory's README for the
...
- `requirements-extended.txt` begins with `-r requirements.txt`, but **no
  ...
  was reconstructed from the actual import graph of `code/pipeline.py`.
...
## fisher/  (was projects/culinary-corridors)

Three overlapping packages, all retained under `submission/versions/` with none designated
...
| `fisher/submission/versions/fisher-submission/` | `Downloads/culinary_corridors_fisher_submission/culinary_corridors_fisher_submission/` (`code/`, README, BUILD_INSTRUCTIONS, WORK_completed) |
| `submission/versions/storymap-v3-balanced/` | `Downloads/culinary_corridors_storymap_balanced_v3_package/` (`submission/`, `docs/`, `outputs/` → `audits/`, `WORK*.md`) |
| `submission/versions/storymap-v5/` | `Downloads/files (2)/` (v5 BUILD_INSTRUCTIONS + `v4_01`–`v4_06` PNGs → `figures/`) |
| `results/reports/` | `Downloads/culinary_corridors_{complete_final,committee,winner_aligned}_report.pdf` |
...
`fisher/submission/award-materials/` — see that directory's README for the
per-archive breakdown.

Beyond documentation, these archives supplied the entire missing statistical layer:

- `fisher/analysis/` — 13 scripts (Mantel, LISA, robustness,
  case studies, five extensions) from `bridges_final_package.zip`
- `fisher/analysis/working_data/` — distance, similarity and
  residual matrices plus all result JSONs
- `fisher/code/figure-builders/` — 10 figure builders

`culinary_corridors_MASTER.zip` is a full project archive rather than a submission
package; its tree is preserved verbatim under `fisher/submission/award-materials/project-archive/`.
